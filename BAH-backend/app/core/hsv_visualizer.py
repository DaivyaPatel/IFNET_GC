...
class Config:
    ARROW_STEP = 16           # Pixel spacing between arrows (lower = denser)
    ARROW_SCALE = 2.0         # Multiplier for arrow length
    ARROW_THICKNESS = 1       # Arrow shaft thickness in px
...
def flow_to_hsv(flow: np.ndarray, max_mag: float = None) -> np.ndarray:
    """Duotone encoding of flow magnitude. Returns BGR uint8."""
    u, v = flow[..., 0], flow[..., 1]
    mag = np.sqrt(u**2 + v**2)
    ...
    mag_norm = np.clip(mag / max_mag, 0, 1)[..., np.newaxis]
    
    # Duotone theme: Dark grey to Light Grey
    bg_color = np.array([26, 26, 26])
    fg_color = np.array([240, 240, 240])

    bgr = (bg_color * (1 - mag_norm) + fg_color * mag_norm).astype(np.uint8)
...
def draw_arrows(img: np.ndarray, flow: np.ndarray,
    ...
    for y in range(0, h, step):
        for x in range(0, w, step):
            ...
            cv2.arrowedLine(out, (cx, cy), (x2, y2),
                            color=(255, 255, 255),
...
def make_color_wheel(size: int = 180) -> np.ndarray:
    """Duotone radial legend. Returns RGB uint8."""
    yy, xx = np.mgrid[-1:1:size*1j, -1:1:size*1j]
    mag = np.sqrt(xx**2 + yy**2)
    mask = mag <= 1.0

    mag_norm = np.clip(mag, 0, 1)[..., np.newaxis]
    bg_color = np.array([26, 26, 26])
    fg_color = np.array([240, 240, 240])
    
    rgb = (bg_color * (1 - mag_norm) + fg_color * mag_norm).astype(np.uint8)
    rgb[~mask] = 255
    return rgb
...
def visualize_flow(
    ...
    # Convert to block-based (grid) flow
    def blockify_flow(fl: np.ndarray, block_size: int) -> np.ndarray:
        h, w = fl.shape[:2]
        out = np.zeros_like(fl)
        for y in range(0, h, block_size):
            for x in range(0, w, block_size):
                block = fl[y:y+block_size, x:x+block_size]
                out[y:y+block_size, x:x+block_size, 0] = np.mean(block[..., 0])
                out[y:y+block_size, x:x+block_size, 1] = np.mean(block[..., 1])
        return out

    pred = blockify_flow(pred, step)
    if gt is not None:
        gt = blockify_flow(gt, step)
